...
const STORAGE_KEY  = 'hk_dse_lease_layout_v1';
const COUNTER_KEY  = 'hk_property_counter';
const PROPERTY_PREFIX = 'dssjn';
          ...
          <div className={styles.previewDataCol}>
            ...
            <div className={styles.previewDataScroll}>
              <div className={styles.previewDataHeader}>
                <div className={styles.previewDataHeaderSub}>다세대 전세</div>
              ...
              {visibleFields.map(field => (
                <div
                  key={field.id}
                  className={`${styles.previewRow} ${field.id === 'jeonse_price' ? styles.previewRowHighlight : ''}`}
                >
...
let uploadedUrls = [];
    ...
    try {
      ...
      const url     = isEdit ? `/api/dse-lease/${pageId}` : '/api/dse-lease/register';
    ...
    <div className={styles.page}>
      <div className={styles.topbar}>
        <div className={styles.topLeft}>
          ...
          <span className={styles.topTitle}>다세대 전세 — {editMode ? '레이아웃 편집' : isEdit ? '수정' : '등록'}</span>
        </div>
      ...
      <div className={`${styles.canvas} ${showAddPanel ? styles.canvasShift : ''}`}>
                  {!editMode && <BuildingLookup 종류="다세대" 거래유형="전세" onApply={applyBuildingData} />}
