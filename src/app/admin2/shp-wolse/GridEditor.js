...
const STORAGE_KEY = 'hk_shp_wolse_layout_v1';
const COUNTER_KEY = 'hk_property_counter';
const PROPERTY_PREFIX = 'shws';
          ...
          <div className={styles.previewDataCol}>
            ...
            <div className={styles.previewDataScroll}>
              <div className={styles.previewDataHeader}>
                <div className={styles.previewDataHeaderSub}>상가 월세</div>
...
export default function GridEditor({ onBack, isEdit = false, initialValues = null, pageId = null }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const url    = isEdit ? `/api/shp-wolse/${pageId}` : '/api/shp-wolse/register';
    ...
    <div className={styles.page}>
      <div className={styles.topbar}>
        <div className={styles.topLeft}>
          ...
          <span className={styles.topTitle}>상가 월세 — {editMode ? '레이아웃 편집' : isEdit ? '수정' : '등록'}</span>
        </div>
      ...
      <div className={`${styles.canvas} ${showAddPanel ? styles.canvasShift : ''}`}>
                  {!editMode && <BuildingLookup 종류="상가" 거래유형="월세" onApply={applyBuildingData} />}
