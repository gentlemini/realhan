...
const STORAGE_KEY = 'hk_hse_wolse_layout_v1';
const COUNTER_KEY = 'hk_property_counter';
const PROPERTY_PREFIX = 'hssws';
          ...
          <div className={styles.previewDataCol}>
            ...
            <div className={styles.previewDataScroll}>
              <div className={styles.previewDataHeader}>
                <div className={styles.previewDataHeaderSub}>단독주택 월세</div>
              ...
              {visibleFields.map(field => (
                <div
                  key={field.id}
                  className={`${styles.previewRow} ${['deposit', 'monthly_rent'].includes(field.id) ? styles.previewRowHighlight : ''}`}
                >
                  <div className={styles.previewLabel}>{field.label}</div>
                  <div className={`${styles.previewValue} ${['deposit', 'monthly_rent'].includes(field.id) ? styles.previewValuePrice : ''}`}>
                    {renderValue(field)}
                  </div>
                </div>
              ))}
...
export default function GridEditor({ onBack, isEdit = false, initialValues = null, pageId = null }) {
  ...
  const [formValues, setFormValues] = useState({
    ...
    lot_number: { text: '', privacy: '노출', adminMemo: '' },
    property_id: '',
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const url    = isEdit ? `/api/hse-wolse/${pageId}` : '/api/hse-wolse/register';
      ...
      if (res.ok) {
        ...
        if (isEdit) {
          ...
          setFormValues({ location: { text: '', privacy: '노출', adminMemo: '' }, lot_number: { text: '', privacy: '노출', adminMemo: '' }, property_id: generatePropertyId(), direction: { base: '', dir: '동향' } });
        }
      } else {
    ...
    <div className={styles.page}>
      <div className={styles.topbar}>
        <div className={styles.topLeft}>
          ...
          <span className={styles.topTitle}>단독주택 월세 — {editMode ? '레이아웃 편집' : isEdit ? '수정' : '등록'}</span>
        </div>
      ...
      <div className={`${styles.canvas} ${showAddPanel ? styles.canvasShift : ''}`}>
                  {!editMode && <BuildingLookup 종류="단독주택" 거래유형="월세" onApply={applyBuildingData} />}
