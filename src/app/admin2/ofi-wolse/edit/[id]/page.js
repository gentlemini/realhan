...
function itemToFormValues(item) {
  ...
  return {
    ...
    manager_fee:          item.manager_fee,
    facility_fee:         item.facility_fee,
    ...
    offices_count:        item.offices_count,
    shops_count:          item.shops_count,
    restrooms:            item.restrooms,
    current_business:     fromDB(item.current_business),
    recommended_business: fromDB(item.recommended_business),
...
export default function EditPage() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = sessionStorage.getItem('ofi_wolse_edit_item');
      if (stored) {
