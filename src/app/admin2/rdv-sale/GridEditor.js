...
const STORAGE_KEY = 'hk_rdv_sale_layout_v1';
const COUNTER_KEY = 'hk_property_counter';
const PROPERTY_PREFIX = 'rdvm';
          ...
          <div className={styles.previewDataCol}>
            ...
            <div className={styles.previewDataScroll}>
              <div className={styles.previewDataHeader}>
                <div className={styles.previewDataHeaderSub}>재개발 매매</div>
...
export default function GridEditor({ onBack, isEdit = false, initialValues = null, pageId = null }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const url    = isEdit ? `/api/rdv-sale/${pageId}` : '/api/rdv-sale/register';
    ...
    <div className={styles.page}>
      <div className={styles.topbar}>
        <div className={styles.topLeft}>
          ...
          <span className={styles.topTitle}>재개발 매매 — {editMode ? '레이아웃 편집' : isEdit ? '수정' : '등록'}</span>
        </div>
      ...
      <div className={`${styles.canvas} ${showAddPanel ? styles.canvasShift : ''}`}>
                  {!editMode && <BuildingLookup 종류="재개발" 거래유형="매매" onApply={applyBuildingData} />}
