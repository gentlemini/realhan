  ...
  if (field.type === 'direction')
    return (
      <div className={styles.directionRow}>
        ...
        >
          {['동향','서향','남향','북향','남동향','남서향','북동향','북서향'].map(d =>
        ...
        <div className={styles.uploadArea}>
          <label className={styles.uploadBtn}>
            ...
            <input
              ...
              style={{ display: 'none' }}
              disabled={filePreviews.length >= 10}
            />
          </label>
          {filePreviews.length === 0 && (
            <p className={styles.uploadHint}>최대 10장 · 클릭으로 대표사진 선택</p>
          )}
          <div className={styles.photoGrid}>
            {filePreviews.map(({ preview }, idx) => (
              <div
                key={idx}
                className={`${styles.photoItem} ${idx === repIdx ? styles.photoItemActive : ''}`}
  ...
  useEffect(() => {
    ...
    if (radius === 0) {
      ...
      new kakao.maps.Circle({
        ...
        fillColor: '#c19a6b', fillOpacity: 0.15, map,
      });
    }
  }, [lat, lng, radius]);
  ...
  if (field.type === 'mapConfig') {
    return (
      <div className={styles.row} style={{ display: 'block', padding: 0 }}>
        ...
        <AdminMapField
          addressValue={formValues?.location || ''}
          onConfigChange={v => onFormChange?.('map_config', v)}
          initialCoords={formValues?.map_config}
        />
      </div>
    );
  }
...
function PreviewModal({ fields, formValues, filePreviews, repIdx, onClose }) {
  function renderValue(field) {
    ...
    if (field.type === 'multicheck') {
      const arr = v || [];
      return arr.length
        ? <div className={styles.previewTagList}>{arr.map(o => <span key={o} className={styles.previewTag}>{o}</span>)}</div>
        : <span className={styles.previewEmpty}>—</span>;
    }
    if (field.type === 'direction') {
      return v ? <span>{v.base} {v.dir}</span> : <span className={styles.previewEmpty}>—</span>;
    }
            ...
            {pvCurrentSlide ? (
              <>
                ...
                {pvCurrentSlide.type === 'youtube' ? (
                  ...
                  <img src={pvCurrentSlide.preview} alt={`사진 ${pvSlideIdx + 1}`} className={styles.previewPhotoImg} />
                )}
          ...
          <div className={styles.previewDataCol}>
            <div className={styles.previewMapBox}>
              {formValues.map_config?.lat ? (
                <PreviewMap
                  lat={formValues.map_config.lat}
                  lng={formValues.map_config.lng}
                  radius={formValues.map_config.radius || 0}
                />
              ) : (
            ...
            <div className={styles.previewDataScroll}>
              <div className={styles.previewDataHeader}>
                <div className={styles.previewDataHeaderSub}>아파트 전세</div>
              ...
              {visibleFields.map(field => (
                <div
                  key={field.id}
                  className={`${styles.previewRow} ${['apt_name','jeonse_price'].includes(field.id) ? styles.previewRowHighlight : ''}`}
                >
                  <div className={styles.previewLabel}>{field.label}</div>
                  <div className={`${styles.previewValue} ${field.id === 'apt_name' ? styles.previewValueAptName : ''} ${field.id === 'jeonse_price' ? styles.previewValuePrice : ''}`}>
                    {renderValue(field)}
                  </div>
                </div>
              ))}
  ...
  if (field.type === 'mapConfig') {
    return (
      <div ref={setNodeRef} style={{ ...style, display: 'block', padding: 0 }} className={`${styles.row} ${isDragging ? styles.rowDragging : ''}`}>
        ...
        <AdminMapField
          addressValue={formValues?.location || ''}
          onConfigChange={v => onFormChange?.(field.id, v)}
        />
      </div>
    );
  }
  ...
  async function handleSubmit() {
    ...
    try {
      const payload = {
        ...
        imageUrl: uploadedUrls[repIdx] || uploadedUrls[0] || '',
      };
      const url    = isEdit ? `/api/apt-lease/${pageId}` : '/api/apt-lease/register';
      ...
      <div className={styles.topbar}>
        <div className={styles.topLeft}>
          ...
          <span className={styles.topTitle}>
            아파트 전세 — {editMode ? '레이아웃 편집' : isEdit ? '수정' : '등록'}
          </span>
        </div>
        ...
        <div className={styles.addPanel}>
          ...
          <div className={styles.addPanelBody}>
            ...
            {FIELD_TYPES.map(ft => (
              <button
                key={ft.type}
                className={`${styles.typeBtn} ${newType === ft.type ? styles.typeBtnSelected : ''}`}
