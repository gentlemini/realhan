...
function AreaMeta({ property: p }) {
  ...
  const floorText = p.curr_floor ? (p.total_floors ? `${p.curr_floor}/${p.total_floors}층` : `${p.curr_floor}층`) : null;
    ...
    <div className={styles.card} onClick={onClick}>
      <div className={styles.cardThumb}>
        {property.imageUrl ? (
          ...
          <div className={styles.cardImgFallback} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f5f2ee', fontSize: '2.4rem' }}>🏠</div>
        )}
        <div style={{ position: 'absolute', top: 8, left: 8, display: 'flex', gap: 4 }}>
          <span style={{ ...badgeBase, background: catStyle.bg, color: catStyle.color }}>{property.category}</span>
          <span style={{ ...badgeBase, background: txStyle.bg,  color: txStyle.color  }}>{property.transaction}</span>
          {property.map_hidden && (
            <span style={{ ...badgeBase, background: 'rgba(30,30,30,0.7)', color: '#fff' }}>🔒</span>
          )}
        </div>
        <div style={{ position: 'absolute', top: 8, right: 8, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
          {property.property_id && (
            <span style={{ fontSize: '0.65rem', fontWeight: 700, background: 'rgba(0,0,0,0.45)', color: '#fff', padding: '2px 7px', borderRadius: 100 }}>No.{property.property_id}</span>
          )}
        ...
        {property.location && (
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '18px 10px 8px', background: 'linear-gradient(to top, rgba(0,0,0,0.55) 0%, transparent 100%)' }}>
            <span style={{ fontSize: '0.72rem', color: '#fff', textShadow: '0 1px 3px rgba(0,0,0,0.6)' }}>📍 {property.location}</span>
          </div>
        )}
      </div>
      <div className={styles.cardBody}>
        ...
        <p style={{ fontSize: '1.35rem', fontWeight: 800, color: '#a87b51', margin: 0 }}>{price || '가격 미등록'}</p>
