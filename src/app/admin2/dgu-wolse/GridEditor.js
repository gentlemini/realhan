...
const STORAGE_KEY  = 'hk_dgu_wolse_layout_v1';
const COUNTER_KEY  = 'hk_property_counter';
const PROPERTY_PREFIX = 'dgsws';
          ...
          <div className={styles.previewDataCol}>
            ...
            <div className={styles.previewDataScroll}>
              <div className={styles.previewDataHeader}>
                <div className={styles.previewDataHeaderSub}>다가구 월세</div>
              ...
              {visibleFields.map(field => (
                <div
                  key={field.id}
                  className={`${styles.previewRow} ${field.id === 'deposit' ? styles.previewRowHighlight : ''}`}
                >
...
let uploadedUrls = [];
    ...
    try {
      ...
      const url     = isEdit ? `/api/dgu-wolse/${pageId}` : '/api/dgu-wolse/register';
    ...
    <div className={styles.page}>
      <div className={styles.topbar}>
        <div className={styles.topLeft}>
          ...
          <span className={styles.topTitle}>다가구 월세 — {editMode ? '레이아웃 편집' : isEdit ? '수정' : '등록'}</span>
        </div>
      ...
      <div className={`${styles.canvas} ${showAddPanel ? styles.canvasShift : ''}`}>
                  {!editMode && <BuildingLookup 종류="다가구" 거래유형="월세" onApply={applyBuildingData} />}
