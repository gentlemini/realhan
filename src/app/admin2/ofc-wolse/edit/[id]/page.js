...
function itemToFormValues(item) {
  ...
  return {
    ...
    building_use_category: item.building_use_category || '',
    location: {
    ...
    ofc_name:              fromDB(item.ofc_name),
    pyeong:                fromDB(item.pyeong),
    ho: {
      text:      fromDB(item.ho),
      privacy:   item.ho_privacy || '공개',
      adminMemo: fromDB(item.ho_memo),
    },
    ...
    curr_floor:        fromDB(item.curr_floor),
    ...
    building_use_text: fromDB(item.building_use_text),
...
export default function EditPage() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = sessionStorage.getItem('ofc_wolse_edit_item');
      if (stored) {
