...
export default function AdminMapField({ addressValue, onConfigChange, initialCoords }) {
  ...
  function handleRadiusChange(r) {
    ...
    if (markerRef.current) {
      markerRef.current.setMap(r > 0 ? null : (coords ? mapObj.current : null));
    }
  ...
  function handleKeywordSearch(keyword) {
    ...
    places.keywordSearch(keyword, (data, status_) => {
      if (status_ === window.kakao.maps.services.Status.OK) {
        if (data.length === 1) {
          placePin(parseFloat(data[0].y), parseFloat(data[0].x));
          setStatus('✓ 위치 검색 완료');
        } else {
          setResults(data);
          setStatus('');
  ...
  // 소재지 자동 연동 — Geocoder 유지 (500ms debounce)
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      geocoder.addressSearch(addressValue, (result, status_) => {
        if (status_ === window.kakao.maps.services.Status.OK) {
          placePin(parseFloat(result[0].y), parseFloat(result[0].x));
        }
      });
    }, 500);
  ...
  return (
    <div className={styles.wrap}>
      {/* 숨김 상태 오버레이 — map div는 항상 DOM에 유지 */}
      {mapHidden ? (
        <div className={styles.hiddenState}>
          <span className={styles.hiddenIcon}>🗺️</span>
          <span className={styles.hiddenText}>지도 숨김 상태</span>
          <button className={styles.toggleBtn} onClick={() => setMapHidden(false)}>지도 표시</button>
        </div>
      ) : (
        <>
          {/* 검색 바 */}
          <div className={styles.searchBar}>
            ...
            />
            <button className={styles.searchBtn} onClick={() => handleKeywordSearch(searchAddr)}>
              검색
            </button>
      ...
      {/* 지도 — 항상 DOM에 존재, 숨김 시 display:none */}
      <div ref={mapRef} className={styles.map} style={{ display: mapHidden ? 'none' : 'block' }} />
        ...
        <div className={styles.controls}>
          <div className={styles.controlGroup}>
            ...
            <div className={styles.radioBtns}>
              {RADIUS_OPTIONS.map(r => (
                <button
                  key={r}
                  className={`${styles.radioBtn} ${radius === r ? styles.radioBtnActive : ''}`}
            ...
            <span className={styles.controlHint}>
              {radius === 0 ? '정확한 핀 표시' : `핀 숨김 · ${radius}m 반경 원만 표시`}
            </span>
          </div>
          ...
          {coords && (
            <div className={styles.coordInfo}>
              ...
              <span className={styles.coordHint}>핀 드래그로 위치 조정 가능</span>
            </div>
          )}
          ...
          <label className={styles.hiddenToggle}>
            <input type="checkbox" checked={mapHidden} onChange={e => {
              ...
              setMapHidden(v);
              if (coords) onConfigChange?.({ lat: coords.lat, lng: coords.lng, radius, mapHidden: v });
            }} />
            <span>홈페이지에서 지도 숨김</span>
          </label>
        </div>
      )}
    </div>
