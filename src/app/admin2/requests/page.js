  ...
  return (
    <div style={{ maxWidth: 960 }}>
      ...
      {loading ? (
        ...
        <p style={{ color: '#9ca3af', fontSize: '0.9rem' }}>접수된 매물 신청이 없습니다.</p>
      ) : (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
            <thead>
              <tr style={{ background: '#faf9f7', borderBottom: '1px solid #ede8e1' }}>
                {['접수일시', '의뢰인', '연락처', '매물종류', '거래', '소재지', '처리상태'].map((h) => (
            ...
            <tbody>
              {items.map((item, i) => (
                ...
                >
                  ...
                  <td style={{ padding: '10px 14px', color: '#44403c' }}>{item.propertyType || '-'}</td>
                  <td style={{ padding: '10px 14px', color: '#44403c' }}>{item.transaction || '-'}</td>
                  <td style={{ padding: '10px 14px', color: '#44403c', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.location || '-'}</td>
      ...
      {selected && (
        <div style={{ marginTop: 20, background: '#fff', borderRadius: 12, border: '1px solid #ede8e1', padding: '24px 28px', boxShadow: '0 1px 6px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <h3 style={{ fontWeight: 700, fontSize: '0.95rem', color: '#1c1917' }}>{selected.name} 님의 매물접수 상세</h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 24px', fontSize: '0.875rem', marginBottom: 16 }}>
            {[
              ...
              ['이메일', selected.email], ['매물종류', selected.propertyType],
              ['거래종류', selected.transaction], ['소재지', selected.location],
              ['희망가격', selected.price], ['면적', selected.area],
              ['처리상태', selected.status], ['접수일시', fmt(selected.created_time)],
            ].map(([k, v]) => (
              <div key={k}>
          ...
          {selected.memo && (
            <div>
              <p style={{ color: '#78716c', fontWeight: 600, fontSize: '0.875rem', marginBottom: 6 }}>추가 요청사항</p>
              <p style={{ background: '#faf9f7', borderRadius: 8, padding: '12px 14px', fontSize: '0.875rem', color: '#1c1917', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{selected.memo}</p>
            </div>
          )}
        </div>
      )}
