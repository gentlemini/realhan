...
const STORAGE_KEY = 'hk_ofc_sale_layout_v2';
const COUNTER_KEY = 'hk_property_counter';
const PROPERTY_PREFIX = 'ofsm';
  ...
  if (field.type === 'mapConfig') {
    return (
      <div className={styles.row} style={{ display: 'block', padding: 0 }}>
        ...
        <AdminMapField addressValue={formValues?.location || ''} onConfigChange={v => onFormChange?.('map_config', v)} initialCoords={formValues?.map_config} />
      </div>
    );
  }
          ...
          <div className={styles.previewDataCol}>
            ...
            <div className={styles.previewDataScroll}>
              <div className={styles.previewDataHeader}>
                <div className={styles.previewDataHeaderSub}>오피스텔 매매</div>
              ...
              {visibleFields.map(field => (
                <div
                  key={field.id}
                  className={`${styles.previewRow} ${['ofc_name','sale_price','curr_deposit','curr_monthly'].includes(field.id) ? styles.previewRowHighlight : ''}`}
                >
                  <div className={styles.previewLabel}>{field.label}</div>
                  <div className={`${styles.previewValue} ${field.id === 'ofc_name' ? styles.previewValueAptName : ''} ${['sale_price','curr_deposit','curr_monthly'].includes(field.id) ? styles.previewValuePrice : ''}`}>
                    {renderValue(field)}
                  </div>
                </div>
              ))}
  ...
  if (field.type === 'mapConfig') {
    return (
      <div ref={setNodeRef} style={{ ...style, display: 'block', padding: 0 }} className={`${styles.row} ${isDragging ? styles.rowDragging : ''}`}>
        ...
        <AdminMapField addressValue={formValues?.location || ''} onConfigChange={v => onFormChange?.(field.id, v)} />
      </div>
    );
  }
  if (field.type === 'spacer') {
...
export default function GridEditor({ onBack, isEdit = false, initialValues = null, pageId = null }) {
  ...
  const [formValues, setFormValues] = useState({ location: { text: '', privacy: '노출', adminMemo: '' }, address_detail: { text: '', privacy: '노출', adminMemo: '' }, property_id: '', direction: { base: '', dir: '동향' } });
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const url    = isEdit ? `/api/ofc-sale/${pageId}` : '/api/ofc-sale/register';
    ...
    <div className={styles.page}>
      <div className={styles.topbar}>
        <div className={styles.topLeft}>
          ...
          <span className={styles.topTitle}>오피스텔 매매 — {editMode ? '레이아웃 편집' : isEdit ? '수정' : '등록'}</span>
        </div>
      ...
      <div className={`${styles.canvas} ${showAddPanel ? styles.canvasShift : ''}`}>
                  {!editMode && <BuildingLookup 종류="오피스텔" 거래유형="매매" onApply={applyBuildingData} />}
