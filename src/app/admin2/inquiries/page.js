...
function InquiryCard({ item, isSelected, onToggle }) {
  return (
    <div
      onClick={onToggle}
      style={{
    ...
    >
      ...
      {/* 2행: 연락처 */}
      {item.phone && (
        <div style={{ fontSize: '0.82rem', color: '#44403c', marginBottom: 4 }}>
          📞 {item.phone}
        </div>
      )}

      {/* 3행: 관심매물 + 예산 */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px 14px', fontSize: '0.79rem', color: '#78716c', marginBottom: 6 }}>
        {item.type   && <span>관심매물: <strong style={{ color: '#5a3e28' }}>{item.type}</strong></span>}
        {item.budget && <span>예산: <strong style={{ color: '#5a3e28' }}>{item.budget}</strong></span>}
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h3 style={{ fontWeight: 800, fontSize: '0.95rem', color: '#1c1917' }}>{item.name} 님의 문의 상세</h3>
      ...
      {item.message && (
        <div>
          <p style={{ fontSize: '0.72rem', color: '#9d9189', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>문의 내용</p>
...
export default function InquiriesPage() {
  ...
  return (
    <div style={{ maxWidth: 760 }}>
      ...
      {!loading && items.length === 0 && <p style={{ color: '#9ca3af', fontSize: '0.9rem', textAlign: 'center', padding: '40px 0' }}>접수된 문의가 없습니다.</p>}

      {!loading && items.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {items.map(item => (
            <div key={item.id}>
              <InquiryCard item={item} isSelected={selected?.id === item.id} onToggle={() => toggle(item)} />
              {selected?.id === item.id && (
