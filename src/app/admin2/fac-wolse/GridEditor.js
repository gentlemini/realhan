...
const STORAGE_KEY = 'hk_fac_wolse_layout_v1';
const COUNTER_KEY = 'hk_property_counter';
const PROPERTY_PREFIX = 'fcws';
          ...
          <div className={styles.previewDataCol}>
            ...
            <div className={styles.previewDataScroll}>
              <div className={styles.previewDataHeader}>
                <div className={styles.previewDataHeaderSub}>공장/창고 월세</div>
              ...
              {visibleFields.map(field => (
                <div
                  key={field.id}
                  className={`${styles.previewRow} ${["deposit"].includes(field.id) ? styles.previewRowHighlight : ''}`}
                >
                  <div className={styles.previewLabel}>{field.label}</div>
                  <div className={`${styles.previewValue} ${field.id === 'deposit' ? styles.previewValuePrice : ''}`}>
                    {renderValue(field)}
                  </div>
                </div>
              ))}
...
export default function GridEditor({ onBack, isEdit = false, initialValues = null, pageId = null }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const url    = isEdit ? `/api/fac-wolse/${pageId}` : '/api/fac-wolse/register';
    ...
    <div className={styles.page}>
      <div className={styles.topbar}>
        <div className={styles.topLeft}>
          ...
          <span className={styles.topTitle}>공장/창고 월세 — {editMode ? '레이아웃 편집' : isEdit ? '수정' : '등록'}</span>
        </div>
      ...
      <div className={`${styles.canvas} ${showAddPanel ? styles.canvasShift : ''}`}>
                  {!editMode && <BuildingLookup 종류="공장" 거래유형="월세" onApply={applyBuildingData} />}
