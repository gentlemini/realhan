...
const STORAGE_KEY = 'hk_ofc_lease_layout_v2';
const COUNTER_KEY = 'hk_property_counter';
const PROPERTY_PREFIX = 'ofsjn';
          ...
          <div className={styles.previewDataCol}>
            ...
            <div className={styles.previewDataScroll}>
              <div className={styles.previewDataHeader}>
                <div className={styles.previewDataHeaderSub}>오피스텔 전세</div>
              ...
              {visibleFields.map(field => (
                <div
                  key={field.id}
                  className={`${styles.previewRow} ${['ofc_name','jeonse_price'].includes(field.id) ? styles.previewRowHighlight : ''}`}
                >
                  <div className={styles.previewLabel}>{field.label}</div>
                  <div className={`${styles.previewValue} ${field.id === 'ofc_name' ? styles.previewValueAptName : ''} ${field.id === 'jeonse_price' ? styles.previewValuePrice : ''}`}>
                    {renderValue(field)}
                  </div>
                </div>
              ))}
...
export default function GridEditor({ onBack, isEdit = false, initialValues = null, pageId = null }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const url    = isEdit ? `/api/ofc-lease/${pageId}` : '/api/ofc-lease/register';
    ...
    <div className={styles.page}>
      <div className={styles.topbar}>
        <div className={styles.topLeft}>
          ...
          <span className={styles.topTitle}>오피스텔 전세 — {editMode ? '레이아웃 편집' : isEdit ? '수정' : '등록'}</span>
        </div>
      ...
      <div className={`${styles.canvas} ${showAddPanel ? styles.canvasShift : ''}`}>
                  {!editMode && <BuildingLookup 종류="오피스텔" 거래유형="전세" onApply={applyBuildingData} />}
