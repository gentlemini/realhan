          ...
          <div className={styles.previewDataCol}>
            ...
            <div className={styles.previewDataScroll}>
              <div className={styles.previewDataHeader}>
                <div className={styles.previewDataHeaderSub}>아파트 매매</div>
              ...
              {visibleFields.map(field => (
                <div
                  key={field.id}
                  className={`${styles.previewRow} ${['apt_name','sale_price'].includes(field.id) ? styles.previewRowHighlight : ''}`}
                >
                  <div className={styles.previewLabel}>{field.label}</div>
                  <div className={`${styles.previewValue} ${field.id === 'apt_name' ? styles.previewValueAptName : ''} ${field.id === 'sale_price' ? styles.previewValuePrice : ''}`}>
                    {renderValue(field)}
                  </div>
                </div>
              ))}
...
export default function GridEditor({ onBack, isEdit = false, initialValues = null, pageId = null }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const url    = isEdit ? `/api/apt-sale/${pageId}` : '/api/apt-sale/register';
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.topbar}>
        <div className={styles.topLeft}>
          ...
          <span className={styles.topTitle}>
            아파트 매매 — {editMode ? '레이아웃 편집' : isEdit ? '수정' : '등록'}
          </span>
        </div>
      ...
      <div className={`${styles.canvas} ${showAddPanel ? styles.canvasShift : ''}`}>
        {!editMode && (
          <BuildingLookup 종류="아파트" 거래유형="매매" onApply={applyBuildingData} />
        )}
