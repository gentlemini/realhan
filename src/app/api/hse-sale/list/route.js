...
const DB_ID = '646c7f5626084bea9731e115a59e6203';
  ...
  try {
    ...
    const items = results.map(page => {
      ...
      return {
        ...
        location:            gR(p['소재지']),
        lot_number:          gR(p['번지']),
        lot_number_privacy:  gS(p['번지_공개여부']),
        lot_number_memo:     gR(p['번지_메모']),
        location_privacy:    gS(p['소재지_공개여부']),
        location_memo:       gR(p['소재지_메모']),
        address_public:      gS(p['지번노출여부']),
        building_type:       gS(p['건물유형']),
        total_floor_area:    gR(p['연면적_㎡']),
        land_area:           gR(p['대지면적_㎡']),
        building_area:       gR(p['건축면적_㎡']),
        exclusive_area:      gR(p['전용면적_㎡']),
        sale_price:          gN(p['매매가격_만원']),
        curr_jeonse:         gN(p['현전세보증금_만원']),
        curr_monthly_rent:   gN(p['현전월세_만원']),
    ...
    console.error('[hse-sale/list]', err);
