...
function makePinDiv(name, onClick) {
  ...
  if (name) {
    ...
    label.style.cssText = 'background:#fff;border:1.5px solid #ef4444;border-radius:6px;padding:4px 8px;font-size:11px;font-weight:700;color:#111;box-shadow:0 2px 6px rgba(0,0,0,0.18);margin-bottom:4px;width:max-content;max-width:160px;white-space:pre-wrap;word-break:break-word;overflow-wrap:break-word;line-height:1.5;max-height:52px;overflow:hidden;text-align:center;';
...
function PreviewModal({ item, onClose }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/property-detail/${item.id}`)
      .then(r => r.json()).then(d => { setDetail(d); setMemoVal(d?.admin_memo || ''); }).catch(() => setDetail(null)).finally(() => setDetLoading(false));
  }, [item.id]);
  ...
  const handleSaveSession = useCallback(async () => {
    ...
    try {
      await fetch('/api/map-pins', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ page: '금액지도', toSession: name }),
      });
  ...
  const handleRenameSession = useCallback(async () => {
    ...
    try {
      await fetch('/api/map-pins', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ page: '금액지도', fromSession: viewSession, toSession: name }),
      });
  ...
  const handleDeleteSession = useCallback(async () => {
    ...
    try {
      await fetch('/api/map-pins', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ page: '금액지도', session: viewSession }),
      });
