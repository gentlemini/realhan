...
export async function POST(req) {
  try {
    const { name, phone, email, type, budget, message } = await req.json();
    if (!name || !phone || !message) {
      return Response.json({ error: '이름, 연락처, 문의내용은 필수입니다.' }, { status: 400 });
    }
    ...
    const properties = {
      '이름':       { title: [{ text: { content: String(name) } }] },
      '연락처':     addR(phone),
      '이메일':     addR(email),
      '예산':       addR(budget),
      '문의내용':   addR(message),
    ...
    if (type) properties['관심매물종류'] = addS(type);
