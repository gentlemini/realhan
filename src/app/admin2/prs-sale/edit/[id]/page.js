...
function itemToFormValues(item) {
  ...
  return {
    ...
    apt_name:         fromDB(item.apt_name),
    pyeong:           fromDB(item.pyeong),
    supply_area:      item.supply_area,
    exclusive_area:   item.exclusive_area,
    presale_type:     item.presale_type || '',
    has_presale_price: item.has_presale_price || '',
    ...
    unit_parking:     item.unit_parking,
    building_use:     fromDB(item.building_use),
    inspection_date:  fromDB(item.inspection_date),
    opt_ac:           Array.isArray(item.opt_ac) ? item.opt_ac : [],
    opt_general:      Array.isArray(item.opt_general) ? item.opt_general : [],
    opt_security:     Array.isArray(item.opt_security) ? item.opt_security : [],
    opt_extra:        Array.isArray(item.opt_extra) ? item.opt_extra : [],
    opt_parking:      Array.isArray(item.opt_parking) ? item.opt_parking : [],
...
export default function EditPage() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = sessionStorage.getItem('prs_sale_edit_item');
      if (stored) {
