...
function itemToFormValues(item) {
  ...
  return {
    ...
    lot_number: {
      text:      fromDB(item.lot_number),
      privacy:   item.lot_number_privacy || '노출',
      adminMemo: fromDB(item.lot_number_memo),
    },
    address_public: item.address_public || '',
    building_type:  item.building_type || '',
    supply_area:    item.supply_area,
    exclusive_area: item.exclusive_area,
    land_share:     fromDB(item.land_share),
...
export default function EditPage() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = sessionStorage.getItem('hse_lease_edit_item');
      if (stored) {
